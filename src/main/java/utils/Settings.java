...
public class Settings {
     ...
     * @return 
     *  a connection string formatted like this: "jdbc:mysql://localhost:3306/startcode"
     */
    public static String getDEV_DBConnection(){
        intializeProperties();
        return String.format("jdbc:mysql://%s:%s/%s?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC&&useSSL=false",props.getProperty("db.server"),props.getProperty("db.port"),props.getProperty("db.database"));
    }
    
    /**
     * Utility method that builds the TEST-connection string using the property values: db.server , db.port and db.database
     * @return 
     *  a connection string formatted like this: "jdbc:mysql://localhost:3306/startcode_test"
     */
    public static String getTEST_DBConnection(){
        ...
        return String.format("jdbc:mysql://%s:%s/%s?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC&&useSSL=false",server,port,props.getProperty("dbtest.database"));
    }
